...
def download_excel():
    ...
    def set_cell(r, c, value, bold=False, align="left"):
    ...
    def merge_and_set(r1, c1, r2, c2, value, bold=False, align="left"):
    ...
    for app in apps:
        ...
        # Basic info
        set_cell(row + 1, 3, "\uC774\uB984", bold=True, align="center")
        merge_and_set(row + 1, 4, row + 1, 8, app.name or "")
        set_cell(row + 2, 3, "\uC0DD\uB144\uC6D4\uC77C", bold=True, align="center")
        merge_and_set(row + 2, 4, row + 2, 8, app.birth.strftime('%Y-%m-%d') if app.birth else "")
        set_cell(row + 3, 3, "\uC5F0\uB77D\uCC98", bold=True, align="center")
        merge_and_set(row + 3, 4, row + 3, 8, app.phone or "")
        set_cell(row + 4, 3, "\uC774\uBA54\uC77C", bold=True, align="center")
        merge_and_set(row + 4, 4, row + 4, 8, app.email or "")
        set_cell(row + 5, 3, "\uC8FC\uC18C", bold=True, align="center")
        merge_and_set(row + 5, 4, row + 5, 8, app.address or "")

        # Body/size
        set_cell(row + 6, 3, "\uC2E0\uCCB4\uC815\uBCF4", bold=True, align="center")
        ...
        set_cell(row + 7, 3, "\uC0C1\uC138\uC0AC\uC774\uC988", bold=True, align="center")
        ...
        # Work conditions
        set_cell(row + 8, 3, "\uADFC\uBB34\uC870\uAC74", bold=True, align="center")
        merge_and_set(
            row + 8, 4, row + 8, 8,
            f"\uD615\uD0DC:{app.shift or '-'} / \uBC29\uC2DD:{app.posture or '-'}"
        )
        set_cell(row + 9, 3, "\uAC00\uB2A5\uC5EC\uBD80", bold=True, align="center")
        merge_and_set(
            row + 9, 4, row + 9, 8,
        ...
        set_cell(row + 10, 3, "\uD76C\uB9DD\uC77C\uC815", bold=True, align="center")
        unknown = "\uBBF8\uC815"
        ...
        set_cell(row + 11, 3, "\uC0C1\uD0DC", bold=True, align="center")
        merge_and_set(row + 11, 4, row + 11, 8, app.status or "")

        # Career section
        set_cell(row + 12, 3, "\uACBD\uB825\uC0AC\uD56D", bold=True, align="center")
        careers = []
